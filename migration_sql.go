...
func runSQLMigration(db *sql.DB, tableName string, scriptFile string, v int64, direction bool) error {
	...
	if useTx {
		...
		if _, err := tx.Exec(GetDialect().insertVersionSQL(tableName), v, direction); err != nil {
			tx.Rollback()
			return err
		}
	...
	if _, err := db.Exec(GetDialect().insertVersionSQL(tableName), v, direction); err != nil {
		return err
	}
