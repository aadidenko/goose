...
func (m *Migration) Up(db *sql.DB, tableName string) error {
	if err := m.run(db, tableName, true); err != nil {
		return err
	}
...
func (m *Migration) Down(db *sql.DB, tableName string) error {
	if err := m.run(db, tableName, false); err != nil {
		return err
	}
	log.Println("OK   ", filepath.Base(m.Source))
	return nil
...
func (m *Migration) run(db *sql.DB, tableName string, direction bool) error {
	switch filepath.Ext(m.Source) {
	case ".sql":
		if err := runSQLMigration(db, tableName, m.Source, m.Version, direction); err != nil {
	...
	case ".go":
		...
		if _, err := tx.Exec(GetDialect().insertVersionSQL(tableName), m.Version, direction); err != nil {
			tx.Rollback()
			return err
		}
